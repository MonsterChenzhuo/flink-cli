...
7. 如果出现 `sink_busy_upstream_backpressure`，不要因为 sink 自身 backpressure=ok 就判断无问题；它通常表示 sink/外部系统写入吞吐导致上游反压。Doris 场景优先读取 `evidence.doris_sink_metrics.summary` 里的 `per_flush_*`、`load_time_ms_*`、`write_data_time_ms_*`，再分析批次大小、checkpoint 周期和 sink 并发。
